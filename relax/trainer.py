...
class Trainer:
    ...
    def init(
        self, rng: Union[jax.random.PRNGKey, int], *args, **kwargs
    ) -> TrainingState:
        model_params = self.model.init(rng, *args, **kwargs)
        optimizer_state = self.optimizer.init(model_params)
        state = TrainingState(rng, model_params, optimizer_state)
    ...
    def train(
        ...
        loss_fn: Callable,
        ds: Dataset,
        jit_update_step: bool = True,
        jit_epoch_loop: bool = False,
    ) -> TrainingState:
        def __update(
            state: TrainingState, loss_fn: Callable, batch: Batch
        ) -> Union[TrainingState, float]:
        ...
        def __update_epoch(
            state: TrainingState, config: TrainingConfig, loss_fn: Callable, ds: Dataset
        ) -> TrainingState:
        ...
        update_epoch = (
            jax.jit(__update_epoch, static_argnums=(1, 2))
            if jit_epoch_loop
            else __update_epoch
        )
